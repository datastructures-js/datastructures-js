...
class LinkedList {
     ...
     */
    addAfter(value, newValue, current = this.head) {
        if (current === null) {
     ...
     * @param {LinkedListNode} [current=head] - memoizes current node
     */
    addBefore(value, newValue, prev = null, current = this.head) {
        if (current === null) {
            throw new Error('node ' + value + ' not found');
        }
     ...
     * @param {LinkedListNode} [current=head] - memoizes current node
     */
    remove(value, prev = null, current = this.head) {
        if (current !== null) {
     ...
     * @param {LinkedListNode} [current=head] - memoizes current node
     */
    removeLast(prev = null, current = this.head) {
        if (current !== null) {
            if (current.getNext() === null && prev === null) {
                this.head = null;
                this.count--;
            }
            else if (current.getNext() === null && prev !== null) {
                prev.setNext(null);
                this.count--;
            }
            else {
