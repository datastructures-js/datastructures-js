...
class Graph {
     ...
     */
    dfsTraverseEdges(vertex, cb, visited) {
    ...
    dfsTraverse(vertex, cb, visited = {}) {
        if (this.hasVertex(vertex) && !visited[vertex]) {
            cb(vertex);
            visited[vertex] = true;
            this.dfsTraverseEdges(vertex, cb, visited);
        }
    }
     ...
     * @param {Queue} queue
     */
    bfsTraverseEdges(vertex, cb, visited, queue) {
        let edges = this.getEdges(vertex);
        for (let v in edges) {
            if (this.hasEdge(vertex, v) && !visited[v]) {
                queue.enqueue(v);
    ...
    bfsTraverse(vertex, cb) {
        if (this.hasVertex(vertex)) {
            ...
            while (!queue.isEmpty()) {
                ...
                cb(vrtx);
                this.bfsTraverseEdges(vrtx, cb, visited, queue);
            }
        }
    }
