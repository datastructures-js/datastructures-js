...
class Graph {
   ...
   */   
  getEdges(vertex) {
    return this.hasVertex(vertex) ? this._edges[vertex] : null;
  }
   ...
   */
  addVertex(vertex) {
    if (!this._vertices[vertex]) {
      this._vertices[vertex] = true;
      this._edges[vertex] = {};
      this._verticesCount++;
    }
  }
   ...
   */
  removeVertex(vertex) {
    if (this._vertices[vertex] === true) {
      this._vertices[vertex] = false;
      this._verticesCount--;
    }
  }
  ...
  hasEdge(v1, v2) {
    return this.hasVertex(v1) && this.hasVertex(v2)&& 
      this._edges[v1][v2] >= 0 && this._edges[v2][v1] >= 0;
  }
  ...
  findShortestPath(v1, v2) {
    return this._dfsShortestPath.find(v1, v2);
  }
  ...
  _init() {
    ...
    this._dfsShortestPath = new DfsShortestPath(this);             
  }
}
