...
class Heap {
  ...
  _child(left, right) {
    if (left < this._size && right < this._size) {
      if (this._compareChildren(left, right)) {
        return left;
      }
      else {
        return right;
   ...
   * @param {number} i
   * @param {function} isParentAChild
   */
  _bubble(i) {
    let child = i;
    let parent = this._parent(i);
    while (child > 0 && this._isParentAChild(parent, child)) {
  ...
  _fixRoot(parent) {
    ...
    if (child !== null && this._isParentAChild(parent, child)) {
